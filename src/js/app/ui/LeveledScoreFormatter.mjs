...
export var LeveledScoreFormatter = (function () {
  var formatTime = function (ms) {
    ...
    if (years > 0) string += (string.length > 0 ? ' ' : '') + years + 'y'

    if (days > 0) string += (string.length > 0 ? ' ' : '') + days + 'd'

    if (hours > 0) string += (string.length > 0 ? ' ' : '') + hours + 'h'

    if (minutes > 0) string += (string.length > 0 ? ' ' : '') + minutes + 'm'

    if (seconds > 0 || string.length == 0) string += (string.length > 0 ? ' ' : '') + seconds + 's'
  ...
  class LeveledScoreFormatter {
    ...
    formatStatus(leveledScore) {
      if (leveledScore.fee) return this.formatFee(leveledScore) + ' fee'
    ...
    formatLeveledScore(leveledScore) {
      return (
        'Score ' + this.formatScore(leveledScore) + ' / Level ' + this.formatLevel(leveledScore)
      )
    }
