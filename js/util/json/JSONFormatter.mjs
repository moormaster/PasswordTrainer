  ...
  format(JSONStr) {
    ...
    while (JSONStr.length > startIndex) {
      ...
      while (JSONStr.length > currentIndex) {
        ...
        if (JSONStr[currentIndex] == '"') isInsideStringLiteral = !isInsideStringLiteral

        // move cursor until first occurence of ',',  '{' or '}'
        if (!isInsideStringLiteral && ',{}'.indexOf(JSONStr[currentIndex]) >= 0) break
      ...
      if (JSONStr.length <= startIndex) continue
