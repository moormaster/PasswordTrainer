...
export var LeveledScore = (function () {
  var fib = function (i) {
    if (i == 0) return 0
  ...
  var getLockHoursLeft = function (level, lastSuccessDate, readDate) {
    ...
    if (!lockHoursLeft || lockHoursLeft < 0) lockHoursLeft = 0
  ...
  var getFeeHoursPassed = function (level, lastSuccessDate, readDate) {
    ...
    if (!feeHoursPassed || feeHoursPassed < 0) feeHoursPassed = 0
  ...
  class LeveledScore {
    ...
    addSuccessfulAttempt(dateOfAttempt) {
      if (dateOfAttempt == null) dateOfAttempt = new Date().getTime()

      var lastSuccessLevel = getLevel(this.scoreData.lastSuccessScore)
      var lockHoursLeft = getLockHoursLeft(
        lastSuccessLevel,
        this.scoreData.lastSuccessTimestamp,
        dateOfAttempt,
      )

      if (lockHoursLeft > 0) return false
    ...
    getFee(readDate) {
      if (this.scoreData.lastSuccessTimestamp == null) return 0

      var lastSuccessLevel = getLevel(this.scoreData.lastSuccessScore)
      var feeHoursPassed = Math.floor(
        getFeeHoursPassed(lastSuccessLevel, this.scoreData.lastSuccessTimestamp, readDate),
      )

      var fee = feeHoursPassed * this.feePerHour

      if (fee > this.scoreData.lastSuccessScore) return this.scoreData.lastSuccessScore
